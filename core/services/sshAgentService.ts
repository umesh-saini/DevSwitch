...
export class SSHAgentService {
  async addKeyToAgent(params: {
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to add key to agent",
      };
    }
  }
  ...
  private async addKeyToAgentWindows(params: {
    ...
    try {
      const serviceCheck = await execAsync("sc query ssh-agent").catch(
        () => null,
      );

      if (!serviceCheck || !serviceCheck.stdout.includes("RUNNING")) {
        try {
          await execAsync("Start-Service ssh-agent", {
